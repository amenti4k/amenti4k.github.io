...
## Postscript: scoring this, three months inside (July 2026)

*I joined. What follows uses public information only, and the April text above is unchanged. The whole point of writing predictions down is that you have to stand next to them later.*

The bet I flagged as most fragile broke within eight weeks. In May, Anthropic announced a $1.5B enterprise-services venture with Blackstone, Hellman & Friedman, and Goldman; OpenAI answered seven days later with a $4B+ deployment company built explicitly on the forward-deployed model, and both labs had already shipped named healthcare products in January (OpenAI for Healthcare live at Cedars-Sinai and HCA, Claude for Healthcare launched at JPM). The cultural claim survived on a technicality that matters: neither lab does this work in-house. Both built arm's-length vehicles with other people's capital, which is exactly what you'd predict if the work is lucrative but repellent to researchers. The strategic comfort did not survive. The complement became a competitor without ceasing to be a supplier, and any plan that assumed a clean lane between labs and deployment now has to win a contested one.

The core thesis got crowded, which is trickier news than being wrong. "Verifiability is the binding constraint" went from a claim I was nervous to type to a consensus position: Jason Wei's verifier's law, Karpathy calling eval design the human bottleneck, and an entire funded industry converting institutional processes into checkable tasks from the outside, with Mercor pitching healthcare RL environments at a ten-figure valuation and Anthropic reportedly budgeting a billion dollars for environments. And the incumbent I should have named in April: Epic launched Agent Factory in March, a no-code agent platform inside the system of record, with revenue-cycle agents already reporting denial reductions at customer sites. In healthcare, Epic occupies the exact position this memo argued was decisive, and it did not need to earn its way inside. The moat was never being right about verification. It's whether deployment six is cheaper than deployment one, and that test now runs on a shorter clock than I gave it.

What held is everything about institutions. Roughly 37 states have moved on healthcare-AI law, nearly all of it mandating human accountability for adverse decisions; CMS's own AI pilot requires that a human clinician issue any denial; liability still lands on people, not algorithms. The deployment gap shows up in the macro data on schedule: a majority of CEOs report no financial benefit from AI yet, and healthcare has the lowest agents-in-production rate of any major sector. And the closing section aged best of all. The first half of 2026 was hard power all the way down: PE capital with guaranteed returns behind OpenAI's deployment company, Blackstone behind Anthropic's, OpenAI equity inside Thrive's holdco. Which leaves the sentence I trusted least as the one still standing, now as a question I work on from inside: the thesis being right and Percepta winning are different claims. Scoring the difference in public, on this page, is the most honest way I know to keep asking it.
